(() => {
  ...
  function resolveCssColorToRgb(cssColor){
    const probe = document.createElement('span'); probe.style.color = cssColor; document.body.appendChild(probe);
    const c = getComputedStyle(probe).color; probe.remove(); const m = c.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/i);
  ...
  function setPinColor(pin, color){
    ...
    if (luminance > 0.62){ titleEl.style.color = '#0f1b2d'; noteEl.style.color = '#1f2d45'; }
    else { titleEl.style.color = '#fff'; noteEl.style.color = '#eaf0ff'; }
  }
  ...
  function applyDistanceStyle(pin){
    ...
    const c = center(); const d = Math.hypot((x+w/2)-c.x, (y+h/2)-c.y); const p = proximityFactor(d);
    const cardScale = 0.92 + (p * 0.20), titleSize = 12 + (p * 3.2), bodySize = 10.5 + (p * 1.6), titleWt = Math.round(540 + (p * 140));
    pin.style.transform = `scale(${cardScale.toFixed(3)})`;
    const title = pin.querySelector('.pin-title input'), note = pin.querySelector('.pin-note input');
    title.style.fontSize = `${titleSize.toFixed(2)}px`; title.style.fontWeight = String(titleWt);
    note.style.fontSize = `${bodySize.toFixed(2)}px`; note.style.fontWeight = '480';
  }

  function refreshSwatches(){
    document.querySelectorAll('.sw').forEach(sw => sw.classList.toggle('active', activePin && activePin.dataset.color === sw.dataset.color));
  }
  ...
  function bindPin(pin){
    pin.addEventListener('pointerdown', (e) => {
      activePin = pin; refreshSwatches();
      pin.classList.add('dragging'); pin.setPointerCapture(e.pointerId);
      const rect = pin.getBoundingClientRect(), surfRect = surface.getBoundingClientRect();
      const offsetX = e.clientX - rect.left, offsetY = e.clientY - rect.top;
      const onMove = (ev) => {
        const w = pin.offsetWidth || rect.width, h = pin.offsetHeight || rect.height;
        const x = ev.clientX - surfRect.left - offsetX, y = ev.clientY - surfRect.top - offsetY;
      ...
      const onUp = (ev) => {
        ...
        pin.removeEventListener('pointermove', onMove); pin.removeEventListener('pointerup', onUp); pin.removeEventListener('pointercancel', onUp);
        applyDistanceStyle(pin); savePin(pin); // no snap/reassign/normalize
      };
      pin.addEventListener('pointermove', onMove); pin.addEventListener('pointerup', onUp); pin.addEventListener('pointercancel', onUp);
    });
  ...
  items.forEach((item) => {
    ...
    pin.style.left = `${item.x}px`; pin.style.top = `${item.y}px`;
  ...
  if (surface.querySelector('.pin')) { activePin = surface.querySelector('.pin'); refreshSwatches(); }
